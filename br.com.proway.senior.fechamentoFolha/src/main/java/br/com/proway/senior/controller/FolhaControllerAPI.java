...
@RestController
@RequestMapping(value = "/folha", produces = "application/json")
public class FolhaControllerAPI {
	...
	public ResponseEntity<List<FolhaDto>> buscarTodasFolhas() {
		...
		List<Folha> listaImprime = folhaController.getAll();
		for (Folha folha : listaImprime) {
			listaFolhaDTO.add(new FolhaDto(folha));
		}
	...
	@PostMapping
	public ResponseEntity<FolhaDto> construirFolhaNormal(IColaboradorFolha colaborador, IPontoFolha ponto,
			ICargoFolha cargo, Bonificacao bonificacao) {
